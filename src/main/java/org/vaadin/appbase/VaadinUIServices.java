...
public class VaadinUIServices implements Serializable {
  ...
  /**
   * Creates a new instance of {@link VaadinUIServices} and sets this instance
   * as data property for the current {@link UI}.
   * 
   * @throws IllegalStateException
   *           if the current UI's data property is alread set, e.g. if this
   *           function is called a second time for the same UI
   */
  public static void startUp() {
    if (UI.getCurrent().getData() != null) {
   ...
   * 
   * @throws IllegalStateException
   *           if the current UI's data property is not a
   *           {@link VaadinUIServices} object or if no such object exists since
   *           {@link #startUp()} has not been called
   */
  public static VaadinUIServices UIServices() {
    Object services = UI.getCurrent().getData();

    if (services == null) {
      throw new IllegalStateException("No valid SessionServices object found for UI " + UI.getCurrent().toString()
          + ". Did you forget to initialize the session service with SessionServices.startUp()?");
    }

    if (!(services instanceof VaadinUIServices)) {
      throw new IllegalStateException(
